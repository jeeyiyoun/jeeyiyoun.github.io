...
function useStyleAndRect(props) {
    ...
    if (!hasEvents(props) && !props.__fromCanvasComponent && props.overflow !== "auto") {
...
/** @internal */
export const FrameWithMotion = forwardRef(function FrameWithMotion(props, ref) {
...
const VisibleFrame = forwardRef(function VisibleFrame(props, forwardedRef) {
    ...
    const wrappedContent = useProvideParentSize(React.createElement(React.Fragment, null,
        backgroundImage ? (React.createElement(BackgroundImageComponent, { image: backgroundImage, containerSize: rect ?? undefined, nodeId: props.id && nodeIdFromString(props.id), layoutId: layoutId })) : null,
        children,
        React.createElement(Border, { ..._border, border: border, layoutId: layoutId })), parentSize);
    return (React.createElement(motion.div, { ...dataProps, ...motionProps, layoutId: layoutId, style: currentStyle, ref: ref, transformValues: transformValues }, wrappedContent));
});
